...
PLOT_DIR = project_root / 'plot' / 'molecule_only' # Changed plot directory path
...
# Configuration Dictionary (Molecule Only)
CONFIG = {
    # Paths (Dynamically generated absolute paths using pathlib)
    "train_csv": project_root / 'data' / 'Train.csv',
    # Removed: "seq_train_feat_path": project_root / 'encode' / 'ESMC' / 'Train_esm_6b.npy',
    ...
    # Removed: "seq_test_feat_path": project_root / 'encode' / 'ESMC' / 'Test_esm_6b.npy',
    "mol_test_feat_path": project_root / 'encode' / 'Uni-Mol2' / 'atomic' / 'Test_unimol_1.1B_atomic_pad.npy',
    "model_dir_base": project_root / 'model_molecule_only', # Base directory for molecule-only models
    "optuna_study_path": project_root / 'model_molecule_only' / 'optuna_study_mol_only.pkl', # Path for molecule-only study
    ...
    # Hyperparameter Search Ranges (Molecule Only - Removed seq/fusion specific)
    "lr_range": (1e-6, 5e-4),
    "weight_decay_range": (1e-4, 1e-1),
    # Removed: "seq_pre_cnn_dim_choices": [128, 256],
    ...
    "wandb_run_name": "Bilingua-DPPIV-MolOnly", # Updated run name for molecule-only

    # CNN Encoder specific Pooling sizes
    # Removed: "seq_pooling_size": 5,
    "mol_pooling_size": 15,
}

# Print configured paths for verification
print("\n--- Configured Paths (Molecule Only) ---")
...
class EncoderCNN(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch, seq_len/mol_len, embedding_dim)
        # Project features first
        x_proj = self.pre_cnn_projection(x) # -> (batch, seq_len/mol_len, projected_dim)
        # Permute for Conv1d: (batch, channels, seq_len/mol_len)
        x_permuted = x_proj.permute(0, 2, 1) # -> (batch, projected_dim, seq_len/mol_len)
...
# --------------------- Molecule Only Model (Using Generic Encoder) ---------------------
class MoleculeModel(nn.Module):
    def __init__(self, mol_embedding_dim, mol_projected_dim,
        ...
        # === Final Classifier ===
        # Input dimension is the output dimension of the molecule encoder's FC layer
    ...
    def forward(self, mol_features):
        ...
        output = self.classifier(mol_repr) # (batch, num_classes)
        return output
...
# --------------------- Data Loading (Molecule Only) ---------------------
def load_molecule_data(train_csv, mol_train_feat_path,
                       test_csv, mol_test_feat_path):
    """Loads molecule features and labels."""
    ...
    assert train_mol_features.shape[0] == len(train_labels), "Train feature/label count mismatch"
    assert test_mol_features.shape[0] == len(test_labels), "Test feature/label count mismatch"

    print(f"Train molecule features shape: {train_mol_features.shape}")
    print(f"Test molecule features shape: {test_mol_features.shape}")
    print("Data loading complete.")

    # Return tuple with molecule features and labels
    return (train_mol_features, train_labels), \
           (test_mol_features, test_labels)


# --------------------- Training and Validation Loops (Molecule Only) ---------------------
def train_molecule_epoch(model, loader, criterion, optimizer, clip_value=CONFIG["clip_value"]):
    model.train()
    running_loss = 0.0
    correct_predictions = 0
    total_samples = 0
    # Unpack molecule features and labels from loader
    for mol_features, labels in loader:
        mol_features, labels = mol_features.to(device), labels.to(device)
        batch_sample_size = mol_features.size(0)
        total_samples += batch_sample_size

        optimizer.zero_grad()
        outputs = model(mol_features) # Pass only molecule features to model
...
def validate_molecule_epoch(model, loader, criterion):
    model.eval()
    running_loss = 0.0
    all_preds = []
    all_labels = []
    ...
    total_samples = 0
    with torch.no_grad():
        # Unpack molecule features and labels
        for mol_features, labels in loader:
            mol_features, labels = mol_features.to(device), labels.to(device)
            batch_sample_size = mol_features.size(0)
            total_samples += batch_sample_size
...
# Note: val_metrics structure should match the output of validate_molecule_epoch
def ensemble_predictions(all_fold_test_probs, test_labels, val_metrics=None, ensemble_method='avg'):
    ...
    # Ensure input is a NumPy array and handle potential None values from failed folds
    valid_probs = [p for p in all_fold_test_probs if p is not None and p.ndim == 2]
    if not valid_probs:
        print("Error: No valid fold probabilities available for ensembling.")
        return None, None, [0.0] * 7

    all_probs_np = np.array(valid_probs)

    if all_probs_np.ndim < 3 or all_probs_np.shape[0] == 0:
        print(f"Error: Invalid shape after filtering Nones for ensembling: {all_probs_np.shape}")
        return None, None, [0.0] * 7 # Return zero metrics

    num_folds, num_samples, num_classes = all_probs_np.shape

    # Check for label consistency
    if num_samples != len(test_labels):
         print(f"Error: Mismatch between number of samples in probabilities ({num_samples}) and test labels ({len(test_labels)}).")
         return None, None, [0.0] * 7
    ...
    if ensemble_method == 'avg':
        ...
        if val_metrics is not None:
             # Filter val_metrics corresponding to the valid folds
             valid_indices = [i for i, p in enumerate(all_fold_test_probs) if p is not None and p.ndim == 2]
             valid_val_metrics = [val_metrics[i] for i in valid_indices if i < len(val_metrics)]

             if len(valid_val_metrics) == num_folds:
                 # Assume val_metrics structure: [acc, sn, sp, pre, mcc, f1, auc] -> MCC is index 4
                 mccs = np.array([m[4] if isinstance(m, (list, tuple)) and len(m) > 4 and m[4] is not None else -1.0 for m in valid_val_metrics])
                 # Handle negative or zero MCCs safely for weighting
                 valid_mccs_vals = mccs[mccs > 0]
                 if len(valid_mccs_vals) > 0:
                     weights = mccs.copy()
                     ...
                     print("Warning: All valid validation MCCs were non-positive. Using equal weights for weighted_avg.")
             else:
                  print(f"Warning: Validation metrics ({len(valid_val_metrics)}) mismatched with valid folds ({num_folds}) after filtering. Using equal weights for weighted_avg.")
        else:
             print(f"Warning: Validation metrics missing. Using equal weights for weighted_avg.")
...
def summarize_ensemble_comparison(all_fold_test_probs, test_labels, val_metrics=None):
    ...
    for method in methods:
        try:
             # Ensure all_fold_test_probs is passed correctly
...
# --------------------- Optuna Objective Function (Molecule Only) ---------------------
def objective_molecule(trial: Trial, train_data_dict):
    """Optuna objective function for molecule-only model."""
    # Unpack pre-loaded data
    train_mol_features = train_data_dict['mol_features']
    train_labels = train_data_dict['labels']

    # Suggest hyperparameters (Molecule only)
    ...
    # Removed seq/fusion related hyperparameters
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_mol_features, train_labels)):
        print(f"  Fold {fold+1}/{CONFIG['optuna_kfold_splits']}")

        # Split molecule features and labels for this fold
        X_mol_train, X_mol_val = train_mol_features[train_index], train_mol_features[val_index]
        y_train, y_val = train_labels[train_index], train_labels[val_index]

        # --- Feature Scaling (Molecule Only) ---
        ...
        # Convert to Tensors (DataLoader expects batch, mol_len, features)
        X_mol_train_tensor = torch.Tensor(X_mol_train)
        X_mol_val_tensor = torch.Tensor(X_mol_val)
        y_train_tensor = torch.LongTensor(y_train)
        y_val_tensor = torch.LongTensor(y_val)

        # Create TensorDataset and DataLoaders (Molecule Only)
        train_dataset_fold = TensorDataset(X_mol_train_tensor, y_train_tensor)
        val_dataset_fold = TensorDataset(X_mol_val_tensor, y_val_tensor)
        ...
        # Instantiate the MoleculeModel
        mol_embedding_dim = train_mol_features.shape[-1] # Get dim from data
        model = MoleculeModel(
            mol_embedding_dim=mol_embedding_dim,
            mol_projected_dim=mol_projected_dim,
        ...
        try:
            for epoch in range(num_epochs):
                train_loss, train_acc = train_molecule_epoch(model, train_loader, criterion, optimizer)
                val_loss, val_acc, _, _, _, val_mcc, _, _ = validate_molecule_epoch(model, val_loader, criterion) # Only need MCC for Optuna
                scheduler.step()
...
# --------------------- Optuna Study Execution (Molecule Only) ---------------------
def run_molecule_optuna():
    """Runs Optuna hyperparameter optimization for molecule-only model."""
    print("\n===== Starting Optuna Hyperparameter Optimization (Molecule Only) =====")

    # Load data ONCE before starting the study (Molecule Only)
    try:
        (train_mol_features, train_labels), _ = load_molecule_data(
            CONFIG["train_csv"], CONFIG["mol_train_feat_path"],
            CONFIG["test_csv"], CONFIG["mol_test_feat_path"]
        )
    ...
    try:
        ...
        study.optimize(lambda trial: objective_molecule(trial, train_data_dict),
    ...
    print("\n===== Optuna Hyperparameter Optimization Finished (Molecule Only) =====")
    ...
    # Save study using path from CONFIG (molecule-only specific path)
    ...
    try:
        plot_optuna_results(study) # Plotting function remains the same, but input is mol-only study
    except Exception as e:
...
# --------------------- Main Training Workflow (Molecule Only) ---------------------
def main_train_molecule(best_params):
    """Main training workflow using K-Fold cross-validation and best hyperparameters (Molecule Only)."""
    print("\n===== Starting Main K-Fold Training & Evaluation (Molecule Only) =====")
    print(f"Using Best Hyperparameters: {best_params}")

    # Ensure model directory exists (molecule-only specific path)
    model_dir = CONFIG["model_dir_base"] # Already points to 'model_mol_only'
    model_dir.mkdir(parents=True, exist_ok=True)
    print(f"Models will be saved in: {model_dir}")

    # Load data (Molecule Only)
    try:
        (train_mol_features, train_labels), \
        (test_mol_features, test_labels) = load_molecule_data(
            CONFIG["train_csv"], CONFIG["mol_train_feat_path"],
            CONFIG["test_csv"], CONFIG["mol_test_feat_path"]
        )
    except Exception as e:
    ...
    # --- Feature Scaling for Test Set (Fit on FULL training data - Molecule Only) ---
    print("Fitting molecule scaler on full training data for test set transformation...")
    ...
    # Extract hyperparameters from best_params (Molecule Only)
    lr = best_params['lr']
    ...
    print(f"\n===== Starting {CONFIG['main_kfold_splits']}-Fold Cross-Validation (Molecule Only) =====")
    for fold, (train_index, val_index) in enumerate(skf.split(train_mol_features, train_labels)):
        print(f"\n--- Fold {fold+1}/{CONFIG['main_kfold_splits']} ---")
        fold_history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': [], 'val_mcc': []}

        # Split features and labels for this fold (Molecule Only)
        X_mol_train, X_mol_val = train_mol_features[train_index], train_mol_features[val_index]
        y_train, y_val = train_labels[train_index], train_labels[val_index]

        # --- Feature Scaling (Inside Fold - Molecule Only) ---
        ...
        # Convert to Tensors (Molecule Only)
        ...
        # Instantiate the model for this fold (MoleculeModel)
        ...
        for epoch in range(num_epochs):
            train_loss, train_acc = train_molecule_epoch(model, train_loader, criterion, optimizer)
            val_loss, val_acc, val_sn, val_sp, val_pre, val_mcc, val_f1, val_auc = validate_molecule_epoch(model, val_loader, criterion)
        ...
        if model_path.exists() and best_epoch != -1:
            ...
            try:
                ...
                # Prepare test dataset loader (using pre-scaled test data - Molecule Only)
                test_mol_tensor = torch.Tensor(test_mol_features_scaled)
                test_labels_tensor = torch.LongTensor(test_labels)
                test_dataset = TensorDataset(test_mol_tensor, test_labels_tensor)
                ...
                with torch.no_grad():
                    for mol_feat_test, label_test in test_loader:
                        mol_feat_test = mol_feat_test.to(device)
                        outputs_test = model_load(mol_feat_test) # Pass only molecule features
                ...
                wandb.log({
                 ...
                 # Append None or handle failure appropriately for aggregation later
                 all_fold_test_probs.append(None)
             ...
             all_fold_test_probs.append(None) # Append None if fold fails or model not found
    ...
    print("\n===== K-Fold Cross-Validation Finished (Molecule Only) =====")

    # --- Aggregate and Summarize Results ---
    print("\n===== Overall Training & Evaluation Summary (Molecule Only) =====")
    # No fusion method to print

    # Summarize Validation Metrics (using the best epoch from each fold)
    valid_val_folds = [m for m in all_fold_val_metrics if m is not None and len(m) == 7 and m[4] > -1.0] # Check MCC validity
    if valid_val_folds:
    ...
    valid_test_folds_metrics = [m for m in all_fold_test_metrics if m is not None and len(m) == 7 and m[4] > -1.0]
    ...
    # --- Ensemble Evaluation ---
    print("\n===== Final Ensemble Evaluation on Test Set (Molecule Only) =====")
    ...
    print("\n===== Generating Visualizations (Molecule Only) =====")
    try:
        # Plot average training/validation curves
        plot_training_history(fold_histories, filename=PLOT_DIR / f"training_curves_mol_only.png")

        # Plot ROC curves for folds and ensemble
        plot_roc_curves(all_fold_test_probs, test_labels, ensemble_results_dict, best_ensemble_method, filename=PLOT_DIR / f"roc_curves_mol_only.png")
        ...
        if best_ensemble_preds is not None:
             plot_confusion_matrix(test_labels, best_ensemble_preds, display_labels=["Non-inhibitor", "Inhibitor"],
                                   filename=PLOT_DIR / f"confusion_matrix_{best_ensemble_method}_mol_only.png",
                                   title=f"Confusion Matrix (Mol Only - Ensemble: {best_ensemble_method})")
        else:
        ...
        if best_ensemble_probs is not None:
             plot_probability_distribution(test_labels, best_ensemble_probs[:, 1], # Use probability of positive class
                                           filename=PLOT_DIR / f"probability_dist_{best_ensemble_method}_mol_only.png",
                                           title=f"Probability Distribution (Mol Only - Ensemble: {best_ensemble_method})")
        else:
        ...
        if valid_test_folds_metrics: # Check if there are valid metrics to plot
            ...
            plot_kfold_metrics_distribution(valid_test_folds_metrics, metric_names,
                                              filename=PLOT_DIR / "kfold_test_metrics_dist_mol_only.png",
                                              title="Distribution of Test Metrics Across K-Folds (Molecule Only)")
        else:
...
def plot_optuna_results(study):
    ...
    try:
        ...
        fig_importance.update_layout(title="Hyperparameter Importance (Optuna - Molecule Only)", title_x=0.5)
        # Use Path object for saving
        filepath = PLOT_DIR / "optuna_param_importances_mol_only.png" # Changed filename
    ...
    # This plot is not relevant for molecule-only, so skip it or adapt if other categorical params exist
    print("Skipping fusion performance plot as it's not applicable for molecule-only.")
...
def plot_training_history(fold_histories, filename="training_curves.png"):
    ...
    axes[0].set_title('Average Training and Validation Loss (Molecule Only)', fontsize=14) # Updated title
    ...
    axes[1].set_title('Average Validation MCC (Molecule Only)', fontsize=14) # Updated title
...
def plot_roc_curves(all_fold_test_probs, test_labels, ensemble_results_dict, best_ensemble_method, filename="roc_curves.png"):
    ...
    # Filter out None values from all_fold_test_probs before processing
    valid_probs = [p for p in all_fold_test_probs if p is not None and p.ndim == 2 and p.shape[0] == len(test_labels)]
    num_valid_folds = len(valid_probs)

    # Plot ROC for each valid fold
    for i, probs in enumerate(valid_probs):
         ...
         aucs.append(roc_auc)
         # Find original fold index (assuming order is preserved)
         original_fold_index = [idx for idx, p_orig in enumerate(all_fold_test_probs) if np.array_equal(p_orig, probs)][0]
         plt.plot(fpr, tpr, lw=1, alpha=0.3, label=f'Fold {original_fold_index+1} (AUC = {roc_auc:.2f})')
    ...
    # Plot Mean ROC across valid folds
    if tprs:
    ...
    if best_ensemble_method and best_ensemble_method in ensemble_results_dict and valid_probs:
        # Use the simple average probability of *valid* folds for plotting the ensemble ROC
        probs_ensemble = np.mean(np.array(valid_probs), axis=0)
        ...
        if probs_ensemble.ndim == 2 and probs_ensemble.shape[1] == 2:
            ...
            print(f"Could not plot ROC for ensemble method '{best_ensemble_method}' due to invalid ensemble probabilities.")
    ...
    plt.title('Receiver Operating Characteristic (ROC) Curves (Molecule Only)', fontsize=14) # Updated title
...
# --------------------- Main Execution Block (Molecule Only) ---------------------
if __name__ == "__main__":
    print("Running in Molecule Only Mode (UniMol) - Ablation")

    # Step 1: Run Optuna hyperparameter optimization
    study = run_molecule_optuna()

    if study and study.best_trial and study.best_trial.value > -1.0 : # Ensure a valid best trial exists
        best_params = study.best_trial.params
        print("\nOptuna found best parameters (Molecule Only):")
        print(best_params)
        ...
        print(f"\n--- Proceeding with final {CONFIG['main_kfold_splits']}-Fold Training using Best Config (Molecule Only) ---")

        # Call main_train_molecule with the best params from the study
        all_fold_test_probs, test_labels, final_best_ensemble_method, final_metrics, best_ensemble_preds, best_ensemble_probs, valid_test_folds_metrics = main_train_molecule(best_params)

        # Final Summary based on the main training run
        if final_best_ensemble_method and final_metrics:
            print("\n===== FINAL OVERALL PERFORMANCE (Molecule Only) =====")
            ...
            model_save_path = CONFIG["model_dir_base"] # Already points to mol_only dir
            ...
            # --- Generate Additional Visualizations (Already done in main_train_molecule) ---
            # Visualizations are generated at the end of main_train_molecule

...
PLOT_DIR.mkdir(exist_ok=True) # Create plot directory if it doesn't exist
PLOT_DPI = 300
...
# Generate some colors in the same color family if needed (e.g., lighter/darker versions)
...
CONFIG = {
    ...
    "mol_train_feat_path": project_root / 'encode' / 'Uni-Mol2' / 'atomic' / 'Train_unimol_1.1B_atomic_pad.npy',
    "test_csv": project_root / 'data' / 'Test.csv',
    "seq_test_feat_path": project_root / 'encode' / 'ESMC' / 'Test_esm_6b.npy',
    "mol_test_feat_path": project_root / 'encode' / 'Uni-Mol2' / 'atomic' / 'Test_unimol_1.1B_atomic_pad.npy',
    "model_dir_base": project_root / 'model', # Base directory for models
    "optuna_study_path": project_root / 'model' / 'optuna_study_2modal.pkl', # Unified path for 2-modal study
    ...
    # Hyperparameter Search Ranges (Now includes fusion_method and n_heads)
    "lr_range": (1e-6, 5e-4),
    "weight_decay_range": (1e-4, 1e-1),
    "seq_pre_cnn_dim_choices": [128, 256],
    "mol_pre_cnn_dim_choices": [64, 128],
    "conv1_out_choices": [16, 32, 64],
    "conv2_out_choices": [32, 64, 128],
    "kernel_size_range": (2, 5),
    "dropout_rate_range": (0.4, 0.8),
    "batch_size_choices": [16, 32, 64],
    "fusion_method_choices": ['concat', 'attention', 'gated'],
    "attention_heads_choices": [2, 4], # Only relevant if fusion_method is attention

    # Model Architecture Config (Fixed dimensions after encoders/fusion)
    "final_encoder_fc_dim": 128, # Dimension output by each encoder's final FC layer
    "final_fusion_hidden_dim": 128, # Dimension after the main fusion FC layer

    # Wandb Config
    "wandb_project": "your_project_name",
    "wandb_entity": "your_wandb_entity",
    "wandb_run_name": "Bilingua-DPPIV-2modal-Refactored", # Updated run name
...
# --------------------- Multi-Modal Fusion Model (Using Generic Encoder) ---------------------
class MultiModalFusionModel(nn.Module):
    def __init__(self, seq_embedding_dim, mol_embedding_dim, # Original dims
                 ...
                 dropout_rate=0.5, n_heads=4):
        super(MultiModalFusionModel, self).__init__()

        self.fusion_method = fusion_method
        self.n_heads = n_heads # Relevant only for attention fusion
        encoder_output_dim = CONFIG["final_encoder_fc_dim"] # e.g., 128
        fusion_hidden_dim = CONFIG["final_fusion_hidden_dim"] # e.g., 128

        # === Encoders (Using the generic EncoderCNN) ===
        ...
        if fusion_method == 'concat':
            ...
            # Make sure n_heads is passed correctly if needed
            ...
            # Attention output is pooled/flattened, so input dim to FC depends on how it's handled
            # Let's flatten the output of attention applied to the concatenated features
            self.fusion_fc = nn.Linear(encoder_output_dim * 2, fusion_hidden_dim)
            ...
            self.fusion_fc = nn.Linear(encoder_output_dim, fusion_hidden_dim) # Gated sum has dim = encoder_output_dim
        else:
    ...
    def forward(self, seq_features, mol_features):
        ...
        # mol_features: (batch, mol_len, mol_emb_dim)
        ...
        mol_repr = self.mol_encoder(mol_features) # (batch, encoder_output_dim)

        # === Apply Fusion ===
        if self.fusion_method == 'concat':
            fused_repr = torch.cat([seq_repr, mol_repr], dim=1) # (batch, encoder_output_dim * 2)
            fused = F.relu(self.fusion_fc(fused_repr))          # (batch, fusion_hidden_dim)
        elif self.fusion_method == 'attention':
            # Prepare for attention: stack features -> (batch, 2, encoder_output_dim)
            # Unsqueeze to add sequence dim of 1 for each modality
            combined = torch.stack([seq_repr, mol_repr], dim=1) # (batch, 2, encoder_output_dim)

            # Self-attention over the two modalities
            attn_out, _ = self.attn(combined, combined, combined) # (batch, 2, encoder_output_dim)

            # Flatten attention output before FC layer
            attn_out_flat = attn_out.reshape(batch_size, -1) # (batch, 2 * encoder_output_dim)
            fused = F.relu(self.fusion_fc(attn_out_flat))     # (batch, fusion_hidden_dim)
        elif self.fusion_method == 'gated':
            gate_seq = torch.sigmoid(self.gate_seq(seq_repr))
            gate_mol = torch.sigmoid(self.gate_mol(mol_repr))
            # Gated sum
            gated_sum = (gate_seq * seq_repr) + (gate_mol * mol_repr) # (batch, encoder_output_dim)
            fused = F.relu(self.fusion_fc(gated_sum))                 # (batch, fusion_hidden_dim)

        # Final classification layers
        fused_dropped = self.fusion_dropout(fused)
        output = self.classifier(fused_dropped) # (batch, num_classes)
        return output
...
# --------------------- Data Loading (2 Modalities) ---------------------
def load_multimodal_data(train_csv, seq_train_feat_path, mol_train_feat_path,
                         test_csv, seq_test_feat_path, mol_test_feat_path):
    """Loads multimodal features and labels."""
    ...
    try:
        ...
        train_mol_features = np.load(mol_train_feat_path)
        test_mol_features = np.load(mol_test_feat_path)
    ...
    assert train_seq_features.shape[0] == train_mol_features.shape[0] == len(train_labels), "Train feature/label count mismatch"
    assert test_seq_features.shape[0] == test_mol_features.shape[0] == len(test_labels), "Test feature/label count mismatch"

    print(f"Train sequence features shape: {train_seq_features.shape}")
    print(f"Train molecular features shape: {train_mol_features.shape}")
    print(f"Test sequence features shape: {test_seq_features.shape}")
    print(f"Test molecular features shape: {test_mol_features.shape}")
    print("Data loading complete.")

    # Return tuple with all features and labels
    return (train_seq_features, train_mol_features, train_labels), \
           (test_seq_features, test_mol_features, test_labels)


# --------------------- Training and Validation Loops (2 Modalities) ---------------------
def train_multimodal_epoch(model, loader, criterion, optimizer, clip_value=CONFIG["clip_value"]):
    model.train()
    running_loss = 0.0
    correct_predictions = 0
    total_samples = 0
    # Unpack two features and labels from loader
    for seq_features, mol_features, labels in loader:
        seq_features, mol_features, labels = seq_features.to(device), mol_features.to(device), labels.to(device)
        batch_sample_size = seq_features.size(0)
        total_samples += batch_sample_size

        optimizer.zero_grad()
        outputs = model(seq_features, mol_features) # Pass two features to model
...
def validate_multimodal_epoch(model, loader, criterion):
    model.eval()
    running_loss = 0.0
    all_preds = []
    all_labels = []
    ...
    total_samples = 0
    with torch.no_grad():
        # Unpack two features and labels
        for seq_features, mol_features, labels in loader:
            seq_features, mol_features, labels = seq_features.to(device), mol_features.to(device), labels.to(device)
            batch_sample_size = seq_features.size(0)
            total_samples += batch_sample_size
...
# --------------------- Optuna Objective Function (Multimodal) ---------------------
def objective_multimodal(trial: Trial, train_data_dict):
    """Optuna objective function, now accepting pre-loaded data."""
    # Unpack pre-loaded data
    train_seq_features = train_data_dict['seq_features']
    train_mol_features = train_data_dict['mol_features']
    train_labels = train_data_dict['labels']

    # Suggest hyperparameters (including fusion method and n_heads)
    ...
    mol_projected_dim = trial.suggest_categorical('mol_proj_dim', CONFIG["mol_pre_cnn_dim_choices"])
    ...
    fusion_method = trial.suggest_categorical('fusion_method', CONFIG["fusion_method_choices"])

    # Suggest n_heads only if attention fusion is chosen
    n_heads = 4 # Default value
    if fusion_method == 'attention':
        n_heads = trial.suggest_categorical('n_heads', CONFIG["attention_heads_choices"])
    else:
        # Log a fixed value or simply don't suggest if not relevant
        # This avoids Optuna trying irrelevant combinations
        n_heads = 4 # Or perhaps better: trial.set_user_attr('n_heads', None) if not needed? Let's stick to fixed default.
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_seq_features, train_labels)):
        ...
        X_mol_train, X_mol_val = train_mol_features[train_index], train_mol_features[val_index]
        y_train, y_val = train_labels[train_index], train_labels[val_index]

        # --- Feature Scaling (Inside Fold) ---
        # Sequence Scaler
        ...
        X_mol_train_shape = X_mol_train.shape
        X_mol_train = mol_scaler.fit_transform(X_mol_train.reshape(-1, X_mol_train_shape[-1])).reshape(X_mol_train_shape)
        X_mol_val_shape = X_mol_val.shape
        X_mol_val = mol_scaler.transform(X_mol_val.reshape(-1, X_mol_val_shape[-1])).reshape(X_mol_val_shape)
        # --- End Scaling ---
        ...
        X_mol_train_tensor = torch.Tensor(X_mol_train)
        X_mol_val_tensor = torch.Tensor(X_mol_val)
        y_train_tensor = torch.LongTensor(y_train)
        y_val_tensor = torch.LongTensor(y_val)

        # Create TensorDataset and DataLoaders
        train_dataset_fold = TensorDataset(X_seq_train_tensor, X_mol_train_tensor, y_train_tensor)
        val_dataset_fold = TensorDataset(X_seq_val_tensor, X_mol_val_tensor, y_val_tensor)
        ...
        # Instantiate the 2-modal model
        seq_embedding_dim = train_seq_features.shape[-1] # Get dim from data
        mol_embedding_dim = train_mol_features.shape[-1]
        model = MultiModalFusionModel(
            seq_embedding_dim=seq_embedding_dim, mol_embedding_dim=mol_embedding_dim,
            seq_projected_dim=seq_projected_dim, mol_projected_dim=mol_projected_dim,
            fusion_method=fusion_method,
            num_classes=CONFIG["num_classes"], conv1_out=conv1_out, conv2_out=conv2_out,
            kernel_size=kernel_size, dropout_rate=dropout_rate, n_heads=n_heads
        ).to(device)
        ...
        try:
            for epoch in range(num_epochs):
                train_loss, train_acc = train_multimodal_epoch(model, train_loader, criterion, optimizer)
                val_loss, val_acc, _, _, _, val_mcc, _, _ = validate_multimodal_epoch(model, val_loader, criterion) # Only need MCC for Optuna
                scheduler.step()
...
def run_multimodal_optuna():
    """Runs Optuna hyperparameter optimization."""
    print("\n===== Starting Optuna Hyperparameter Optimization =====")

    # Load data ONCE before starting the study
    try:
        (train_seq_features, train_mol_features, train_labels), _ = load_multimodal_data(
            CONFIG["train_csv"], CONFIG["seq_train_feat_path"], CONFIG["mol_train_feat_path"],
            CONFIG["test_csv"], CONFIG["seq_test_feat_path"], CONFIG["mol_test_feat_path"]
        )
        # Store data in a dictionary to pass to the objective function
        train_data_dict = {
            'seq_features': train_seq_features,
            'mol_features': train_mol_features,
            'labels': train_labels
        }
    except Exception as e:
    ...
    try:
        ...
        study.optimize(lambda trial: objective_multimodal(trial, train_data_dict),
    ...
    print("\n===== Optuna Hyperparameter Optimization Finished =====")
    ...
    # study.best_trial might raise an error if no trials completed successfully
    ...
    # Save study using path from CONFIG
...
# --------------------- Main Training Workflow (Multimodal) ---------------------
def main_train_multimodal(best_params):
    """Main training workflow using K-Fold cross-validation and best hyperparameters."""
    print("\n===== Starting Main K-Fold Training & Evaluation =====")
    print(f"Using Best Hyperparameters: {best_params}")

    # Ensure model directory exists, create a unique name based on best params (optional)
    # Example: include fusion method in the directory name
    fusion_method = best_params.get('fusion_method', 'unknown_fusion')
    model_dir = CONFIG["model_dir_base"] / f'Bilingua_2Modal_{fusion_method}_final_model'
    model_dir.mkdir(parents=True, exist_ok=True)
    print(f"Models will be saved in: {model_dir}")

    # Load data
    try:
        (train_seq_features, train_mol_features, train_labels), \
        (test_seq_features, test_mol_features, test_labels) = load_multimodal_data(
            CONFIG["train_csv"], CONFIG["seq_train_feat_path"], CONFIG["mol_train_feat_path"],
            CONFIG["test_csv"], CONFIG["seq_test_feat_path"], CONFIG["mol_test_feat_path"]
        )
    except Exception as e:
        print(f"Fatal Error: Data loading failed in main training: {e}")
        return [], [], None, None, None, None # Return empty results

    # --- Feature Scaling for Test Set (Fit on FULL training data) ---
    print("Fitting scalers on full training data for test set transformation...")
    # Sequence Scaler for Test Set
    ...
    # Molecule Scaler for Test Set
    mol_scaler_test = StandardScaler()
    train_mol_shape = train_mol_features.shape
    mol_scaler_test.fit(train_mol_features.reshape(-1, train_mol_shape[-1]))
    test_mol_shape = test_mol_features.shape
    test_mol_features_scaled = mol_scaler_test.transform(
        test_mol_features.reshape(-1, test_mol_shape[-1])
    ).reshape(test_mol_shape)
    ...
    mol_projected_dim = best_params['mol_proj_dim']
    conv1_out = best_params['conv1_out']
    conv2_out = best_params['conv2_out']
    kernel_size = best_params['kernel_size']
    dropout_rate = best_params['dropout_rate']
    batch_size = best_params['batch_size']
    # fusion_method already extracted for model_dir name
    n_heads = best_params.get('n_heads', 4) # Get n_heads if present, else default


    print(f"\n===== Starting {CONFIG['main_kfold_splits']}-Fold Cross-Validation =====")
    for fold, (train_index, val_index) in enumerate(skf.split(train_seq_features, train_labels)):
        ...
        # Molecule Scaler
        mol_scaler_fold = StandardScaler()
        X_mol_train_shape = X_mol_train.shape
        X_mol_train_scaled = mol_scaler_fold.fit_transform(X_mol_train.reshape(-1, X_mol_train_shape[-1])).reshape(X_mol_train_shape)
        X_mol_val_shape = X_mol_val.shape
        X_mol_val_scaled = mol_scaler_fold.transform(X_mol_val.reshape(-1, X_mol_val_shape[-1])).reshape(X_mol_val_shape)
        # --- End Scaling ---

        # Convert to Tensors
        X_seq_train_tensor = torch.Tensor(X_seq_train_scaled)
        X_seq_val_tensor = torch.Tensor(X_seq_val_scaled)
        X_mol_train_tensor = torch.Tensor(X_mol_train_scaled)
        X_mol_val_tensor = torch.Tensor(X_mol_val_scaled)
        y_train_tensor = torch.LongTensor(y_train)
        y_val_tensor = torch.LongTensor(y_val)
        ...
        for epoch in range(num_epochs):
            train_loss, train_acc = train_multimodal_epoch(model, train_loader, criterion, optimizer)
            val_loss, val_acc, val_sn, val_sp, val_pre, val_mcc, val_f1, val_auc = validate_multimodal_epoch(model, val_loader, criterion)
        ...
        if model_path.exists() and best_epoch != -1:
            ...
            try:
                ...
                # Prepare test dataset loader (using pre-scaled test data)
                test_seq_tensor = torch.Tensor(test_seq_features_scaled)
                test_mol_tensor = torch.Tensor(test_mol_features_scaled)
                test_labels_tensor = torch.LongTensor(test_labels)
                test_dataset = TensorDataset(test_seq_tensor, test_mol_tensor, test_labels_tensor)
                ...
                with torch.no_grad():
                    for seq_feat_test, mol_feat_test, label_test in test_loader:
                        seq_feat_test, mol_feat_test = seq_feat_test.to(device), mol_feat_test.to(device)
                        outputs_test = model_load(seq_feat_test, mol_feat_test)
    ...
    print("\n===== Overall Training & Evaluation Summary =====")
    print(f'Best Fusion Method (from Optuna): {fusion_method}')
    ...
    if valid_test_folds_metrics:
        ...
        print(f'  Acc={avg_test_metrics[0]:.2f}%(±{std_test_metrics[0]:.2f}), SN={avg_test_metrics[1]:.4f}(±{std_test_metrics[1]:.4f}), SP={avg_test_metrics[2]:.4f}(±{std_test_metrics[2]:.4f}), Prec={avg_test_metrics[3]:.4f}(±{std_test_metrics[3]:.4f})')
    ...
    try:
        ...
        plot_training_history(fold_histories, filename=PLOT_DIR / f"training_curves_{fusion_method}.png")

        # Plot ROC curves for folds and ensemble
        plot_roc_curves(all_fold_test_probs, test_labels, ensemble_results_dict, best_ensemble_method, filename=PLOT_DIR / f"roc_curves_{fusion_method}.png")
        ...
        if best_ensemble_preds is not None:
             plot_confusion_matrix(test_labels, best_ensemble_preds, display_labels=["Non-inhibitor", "Inhibitor"],
                                   filename=PLOT_DIR / f"confusion_matrix_{best_ensemble_method}.png",
                                   title=f"Confusion Matrix (Ensemble: {best_ensemble_method})")
        else:
        ...
        if best_ensemble_probs is not None:
             plot_probability_distribution(test_labels, best_ensemble_probs[:, 1], # Use probability of positive class
                                           filename=PLOT_DIR / f"probability_dist_{best_ensemble_method}.png",
                                           title=f"Probability Distribution (Ensemble: {best_ensemble_method})")
        else:
        ...
        if valid_test_folds_metrics: # Check if there are valid metrics to plot
            ...
            plot_kfold_metrics_distribution(valid_test_folds_metrics, metric_names,
                                              filename=PLOT_DIR / "kfold_test_metrics_dist.png",
                                              title="Distribution of Test Metrics Across K-Folds")
        else:
...
def plot_optuna_results(study):
    ...
    try:
        ...
        fig_importance.update_layout(title="Hyperparameter Importance (Optuna)", title_x=0.5)
        # Use Path object for saving
        filepath = PLOT_DIR / "optuna_param_importances.png"
    ...
    # Check if 'fusion_method' was actually sampled
    if any('fusion_method' in t.params for t in study.trials):
        try:
            # Prepare data: DataFrame with trial value and fusion method
            data = []
            for t in study.trials:
                if t.state == optuna.trial.TrialState.COMPLETE and t.value is not None:
                    method = t.params.get('fusion_method', 'N/A')
                    data.append({'Fusion Method': method, 'Validation MCC': t.value})
            df_fusion = pd.DataFrame(data)

            if not df_fusion.empty:
                 plt.figure(figsize=(8, 6))
                 sns.boxplot(x='Fusion Method', y='Validation MCC', data=df_fusion, palette=PLOT_PALETTE[:len(df_fusion['Fusion Method'].unique())])
                 plt.title('Validation MCC Distribution by Fusion Method (Optuna Trials)')
                 plt.xlabel('Fusion Method')
                 plt.ylabel('Mean Validation MCC per Trial')
                 plt.xticks(rotation=0)
                 plt.tight_layout()
                 filepath = PLOT_DIR / "optuna_fusion_performance.png"
                 plt.savefig(filepath, dpi=PLOT_DPI)
                 plt.close()
                 print(f"Saved Optuna fusion performance plot to {filepath}")
            else:
                 print("No completed trials with fusion method found for plotting.")

        except Exception as e:
            print(f"Could not generate Optuna fusion performance plot: {e}")
    else:
        print("Skipping Optuna fusion performance plot: 'fusion_method' not in hyperparameters.")
...
def plot_training_history(fold_histories, filename="training_curves.png"):
    ...
    max_epochs = max(len(h['train_loss']) for h in fold_histories.values())
    ...
    axes[0].set_title('Average Training and Validation Loss', fontsize=14)
    ...
    axes[1].set_title('Average Validation MCC', fontsize=14)
...
def plot_roc_curves(all_fold_test_probs, test_labels, ensemble_results_dict, best_ensemble_method, filename="roc_curves.png"):
    ...
    for i, probs in enumerate(all_fold_test_probs):
        if probs.ndim == 2 and probs.shape[1] == 2:
             ...
             print(f"Warning: Skipping ROC plot for Fold {i+1} due to invalid probability shape {probs.shape}")
    ...
    if best_ensemble_method and best_ensemble_method in ensemble_results_dict:
        ...
        # Assuming 'avg' or similar was used if not specifically 'optimized' threshold
        if best_ensemble_method == 'optimized':
            # Optimized threshold changes prediction, but ROC uses probabilities.
            # Use the average probs that the threshold was based on.
            probs_ensemble = np.mean(np.array(all_fold_test_probs), axis=0)
        else:
            # For other methods like avg, weighted_avg, calibrated, etc., recalculate the ensemble probs
            # For simplicity, let's just use the simple average probability for plotting the ensemble ROC
            # This might slightly differ from the AUC reported if a different prob calculation was used by the ensemble method itself
            probs_ensemble = np.mean(np.array(all_fold_test_probs), axis=0)

        if probs_ensemble.ndim == 2 and probs_ensemble.shape[1] == 2:
            ...
            print(f"Could not plot ROC for ensemble method '{best_ensemble_method}' due to invalid probabilities.")
...
def plot_probability_distribution(y_true, y_prob_positive, filename, title):
    ...
    try:
        ...
        print(f"Could not generate probability distribution plot: {e}")
...
def plot_kfold_metrics_distribution(all_fold_metrics, metric_names, filename, title):
    ...
    if not all_fold_metrics:
        ...
        return
    try:
        df_metrics = pd.DataFrame(all_fold_metrics, columns=metric_names)
        ...
        print(f"Could not generate K-Fold metrics distribution plot: {e}")


# --------------------- Main Execution Block ---------------------
if __name__ == "__main__":
    print("Running in 2-Modal Fusion Mode (ESM6B + UniMol) - Refactored")

    # Step 1: Run Optuna hyperparameter optimization (now includes fusion method)
    study = run_multimodal_optuna()

    if study and study.best_trial and study.best_trial.value > -1.0 : # Ensure a valid best trial exists
        best_params = study.best_trial.params
        best_fusion_method = best_params.get('fusion_method', 'N/A')
        print("\nOptuna found best parameters:")
        print(best_params)
        # Log best params found by Optuna to wandb config
        wandb.config.update(best_params, allow_val_change=True)

        # Step 2: Perform final K-Fold training and evaluation using the overall best configuration
        print(f"\n--- Proceeding with final {CONFIG['main_kfold_splits']}-Fold Training using Best Config (Fusion: {best_fusion_method}) ---")

        # Directly call main_train_multimodal with the best params from the study
        # Modify the return values to capture necessary data for plotting
        # Correctly capture the returned list of valid fold metrics
        all_fold_test_probs, test_labels, final_best_ensemble_method, final_metrics, best_ensemble_preds, best_ensemble_probs, valid_test_folds_metrics = main_train_multimodal(best_params)
        ...
        if final_best_ensemble_method and final_metrics:
            ...
            model_save_path = CONFIG["model_dir_base"] / f'Bilingua_2Modal_{best_fusion_method}_final_model'
            ...
            wandb.log({
                ...
                "Overall_Best_Fusion_Method": best_fusion_method, # From Optuna
            ...
            try:
                ...
                if best_ensemble_preds is not None:
                    plot_confusion_matrix(test_labels, best_ensemble_preds, display_labels=["Non-inhibitor", "Inhibitor"],
                                          filename=PLOT_DIR / f"confusion_matrix_{final_best_ensemble_method}.png",
                                          title=f"Confusion Matrix (Ensemble: {final_best_ensemble_method})")
                else:
                    print("Skipping confusion matrix plot: Best ensemble predictions not available.")
                ...
                if best_ensemble_probs is not None:
                    plot_probability_distribution(test_labels, best_ensemble_probs[:, 1], # Use probability of positive class
                                                  filename=PLOT_DIR / f"probability_dist_{final_best_ensemble_method}.png",
                                                  title=f"Probability Distribution (Ensemble: {final_best_ensemble_method})")
                else:
                    print("Skipping probability distribution plot: Best ensemble probabilities not available.")

                # Plot K-Fold Test Metrics Distribution
                # Check if the received variable exists and is not empty
                if valid_test_folds_metrics:
                    metric_names = ["Accuracy", "Sensitivity", "Specificity", "Precision", "MCC", "F1-Score", "AUC"]
                    plot_kfold_metrics_distribution(valid_test_folds_metrics, metric_names,
                                                      filename=PLOT_DIR / "kfold_test_metrics_dist.png",
                                                      title="Distribution of Test Metrics Across K-Folds")
                # Fallback check removed as the return value should be correct now
                # If needed, could re-add a check on all_fold_test_metrics here
                else:
                     print("Skipping K-Fold metrics distribution plot: No valid fold metrics data available from main_train_multimodal.")

            except Exception as e:
                 print(f"Warning: Failed to generate additional plots: {e}")
